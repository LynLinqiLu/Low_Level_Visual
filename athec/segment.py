...
def save_label(rgb, label, save_path):
    ...
    rand = np.dstack((l_rgbR,l_rgbG,l_rgbB)).astype(np.uint8)#Linqi added .astype(np.uint8)
    #add  rand = img_as_ubyte(rand)
    rand = img_as_ubyte(rand)

    save_path1 = misc.make_path(save_path)
    skimage.io.imsave(save_path1, rand)
    
    #cvisualization 2: overlay colors onto original image
